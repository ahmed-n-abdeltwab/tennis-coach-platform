 ...
 * Follows the pattern froasks.md for consistent, simple testing
 *
 * @example
 * ```typescript
...
/**
 * Creates a gateway test setup following the tasks.md pattern.
...
export async function createGatewayTest<TGateway, TService>(
  ...
  const gateway = module.get<TGateway>(config.gatewayClass);
  const service = module.get(config.serviceClass) as jest.Mocked<TService>;
  const mockSocket = createMockSocket();
  const mockServer = createMockServer();

  // Inject mock server into gateway if it has a server property
  if ('server' in (gateway as object)) {
    (gateway as Record<string, unknown>)['server'] = mockServer;
  }

  return { gateway, service, module, mockSocket, mockServer };
}
