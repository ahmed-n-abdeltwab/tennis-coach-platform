...
export default async function globalSetup(): Promise<void> {
  ...
  const databaseUrl = `postgresql://${dbUser}:${dbPassword}@${dbHost}:${dbPort}/${databaseName}?connection_limit=5&pool_timeout=2`;
  ...
  // Only run database setup for integration and e2e tests
  if (databaseSuffix !== 'test') {
